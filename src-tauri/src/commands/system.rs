...
pub fn open_file_location(file_path: String) -> Result<()> {
    ...
    // If path is already a directory, open it directly; otherwise get parent
    ...
    #[cfg(target_os = "macos")]
    {
        // For macOS, try to reveal the file if it exists, otherwise open folder
    ...
    #[cfg(target_os = "windows")]
    {
        // For Windows, try to select the file if it exists, otherwise open folder
        if path.exists() && !path.is_dir() {
            std::process::Command::new("explorer")
                .arg("/select,")
                .arg(&file_path)
