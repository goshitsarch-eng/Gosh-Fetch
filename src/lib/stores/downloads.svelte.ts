...
// Load incomplete downloads that need to be restored
async function loadIncompleteDownloads(): Promise<Download[]> {
  try {
    const db = await Database.load('sqlite:gosh-fetch.db');
    const rows = await db.select<DownloadRow[]>(
    ...
    console.error('Failed to load incomplete downloads:', e);
    return [];
  }
}

// Restore incomplete downloads to the engine on app startup
async function restoreIncompleteDownloads() {
  const incompleteDownloads = await loadIncompleteDownloads();

  for (const download of incompleteDownloads) {
    try {
      // Re-add to engine based on download type
      if (download.downloadType === 'magnet' && download.magnetUri) {
        ...
        console.log(`Restored magnet download: ${download.name}`);
      } else if (download.downloadType === 'torrent' && download.infoHash) {
        // For torrent files, we need the magnet URI or the file
        // If we have a magnet URI, use that
        if (download.magnetUri) {
          ...
          console.log(`Restored torrent download: ${download.name}`);
        }
      } else if (download.url) {
        // HTTP/FTP download
        await invoke('add_download', {
          url: download.url,
          options: { savePath: download.savePath }
        });
        console.log(`Restored HTTP download: ${download.name}`);
      }

      // Remove old entry (engine will create new GID)
      await removeDownloadFromDb(download.gid);
    } catch (e) {
...
// Save any download to database (not just completed ones)
  ...
  try {
    ...
    await db.execute(
      ...
      [
        ...
        download.savePath,
        download.createdAt,
        download.status === 'complete' ? new Date().toISOString() : null,
...
// Remove a download from database
async function removeDownloadFromDb(gid: string) {
  try {
    const db = await Database.load('sqlite:gosh-fetch.db');
...
export async function refreshDownloads() {
  ...
  try {
    ...
    // Save all downloads to database for persistence
...
export async function removeDownload(gid: string, deleteFiles: boolean = false): Promise<void> {
  // Check if download is complete (to keep in history) or active (remove from DB)
  const download = downloads.find(d => d.gid === gid);
  const wasComplete = download?.status === 'complete';

  await invoke('remove_download', { gid, deleteFiles });

  // Remove from DB if not completed (active downloads shouldn't persist after removal)
  if (!wasComplete) {
    await removeDownloadFromDb(gid);
  }
